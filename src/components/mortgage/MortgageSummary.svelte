...
<div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-6">
	...
	<div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
		...
		<div>
			...
			<div class="font-semibold text-gray-900">{formatCurrency(result.extraPaymentPerYear)}</div>
		</div>
	</div>
	<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
		<div>
			<div class="text-sm text-gray-600">Месечен доход</div>
			<div class="font-semibold text-gray-900">{formatCurrency(result.monthlyBudget)}</div>
		</div>
		<div>
			<div class="text-sm text-gray-600">Стойност на имота</div>
			<div class="font-semibold text-gray-900">{formatCurrency(result.loanAmount)}</div>
		</div>
		<div>
			<div class="text-sm text-gray-600">Първоначална вноска</div>
			<div class="font-semibold text-green-600">{formatCurrency(result.downPayment)}</div>
		</div>
		<div>
			<div class="text-sm text-gray-600">Сума за кредитиране</div>
			<div class="font-semibold text-blue-600">{formatCurrency(result.principalAmount)}</div>
		</div>
	</div>
</div>
